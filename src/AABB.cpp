...
AABBPosition AABB::clip(const glm::vec3& start, const glm::vec3& end) const
{
  ...
  if (maxX != MAX_VECTOR && (intersection == MAX_VECTOR || glm::distance(start, maxX) < glm::distance(start, intersection))) 
    intersection = maxX;

  if (minY != MAX_VECTOR && (intersection == MAX_VECTOR || glm::distance(start, minY) < glm::distance(start, intersection)))
    intersection = minY;

  if (maxY != MAX_VECTOR && (intersection == MAX_VECTOR || glm::distance(start, maxY) < glm::distance(start, intersection))) 
    intersection = maxY;

  if (minZ != MAX_VECTOR && (intersection == MAX_VECTOR || glm::distance(start, minZ) < glm::distance(start, intersection))) 
    intersection = minZ;

  if (maxZ != MAX_VECTOR && (intersection == MAX_VECTOR || glm::distance(start, maxZ) < glm::distance(start, intersection))) 
    intersection = maxZ;
