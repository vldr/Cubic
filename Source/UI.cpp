...
bool UI::drawTouchButton(unsigned int flag, float x, float y, float z, const char* text, float width, float height, bool invisible)
{
	if (!invisible)
	{
...
void UI::drawTouchControls()
{
	...
	if (state != State::None && state != State::SelectBlockMenu)
	{
		return;
	}
